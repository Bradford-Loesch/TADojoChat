...
pgtools.dropdb(config, "chat").then(()=>{
  ...
  return pgtools.createdb(config, "chat").then(()=>{
    ...
    return db.any("CREATE TABLE session (sid varchar NOT NULL, sess json NOT NULL, expire timestamp(6) NOT NULL) WITH (OIDS=FALSE); ALTER TABLE session ADD CONSTRAINT session_pkey PRIMARY KEY (sid) NOT DEFERRABLE INITIALLY IMMEDIATE;").then(()=>{//This is used to set up the "session" table.
      ...
      +"CREATE VIEW Room_Priviliges AS select room_id, admin_id, permission_level from room_roles union (select id as room_id, owner_id as admin_id, 4 as permission_level from room); "
      ...
      +"CREATE TABLE Poll (id SERIAL PRIMARY KEY, question varchar NOT NULL, answers varchar[] NOT NULL, created_at timestamp NOT NULL DEFAULT (now()), updated_at timestamp NOT NULL DEFAULT (now()))"//TODO: Answers table
