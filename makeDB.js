...
pgtools.dropdb(config, "chat").then(()=>{
  return pgtools.createdb(config, "chat").then(()=>{
    ...
    return db.any("CREATE TABLE session (sid varchar NOT NULL, sess json NOT NULL, expire timestamp(6) NOT NULL) WITH (OIDS=FALSE); ALTER TABLE session ADD CONSTRAINT session_pkey PRIMARY KEY (sid) NOT DEFERRABLE INITIALLY IMMEDIATE;").then(()=>{//This is used to set up the "session" table.
      return db.any("CREATE TABLE Users (id SERIAL PRIMARY KEY, username varchar NOT NULL, password varchar NOT NULL, email varchar NOT NULL, is_admin boolean NOT NULL DEFAULT false, first_name varchar, last_name varchar, birthday date, location varchar, url varchar, skype varchar, description text, created_at timestamp NOT NULL DEFAULT (now()), updated_at timestamp NOT NULL DEFAULT (now())); "
      +"INSERT INTO Users(id, username, password, email) VALUES (0, '[deleted]', 'notarealuser', 'notarealuser'); "
      +"CREATE TABLE Room (id SERIAL PRIMARY KEY, name varchar NOT NULL, owner integer NOT NULL REFERENCES Users ON UPDATE CASCADE ON DELETE CASCADE NOT NULL, is_private boolean NOT NULL, is_locked boolean NOT NULL, created_at timestamp NOT NULL DEFAULT (now()), updated_at timestamp NOT NULL DEFAULT (now())); "
